;(function (global, factory) {
    ...
    **/
    ...
    function passesInput (ins) {
        return function (item, i) {
            ...
            throw 'passInputContracts(:item) mismatches contract ' + ins[i];
        }
    }
    ...
    **/
    api.hom = function hom (ins, out) {
        ...
        if (!type.isArray(ins) || !ins.every(type.isFunction)) {
            throw 'hom expects ins to be array but saw ' + ins;
        }

        if (!type.isFunction(out)) {
            throw 'hom expects out to be function but saw ' + out;
        }

        return function (fn) {
            if (!type.isFunction(fn)) {
                throw 'hom->f expects fn to be function but saw ' + fn;
            }

            if (fn.length !== ins.length) {
                throw 'hom->f aritiy of fn mismatches inputs length';
            }
            ...
            return arity.aritize(ins.length)(function (args) {
                ...
                if (!out(_result)) {
                    throw 'hom->f->f-> output mismatches contract: ' + _result;
                }
    ...
    **/
    api.multiDispatch = function multiDispatch (defs) {
        ...
        if (!type.isArray(defs)) {
            throw 'multiDispatch expects argument to be array but saw ' + defs;
        }
    ...
    **/
    api.displaceArgs = function displaceArgs (fn, positions) {
        if (!type.isFunction(fn)) {
            throw 'displaceArgs expects argument to be function but saw ' + fn;
        }
        ...
        if (!type.isArray(positions)) {
            throw 'displaceArgs expects second argument to be array but saw ' + positions;
        }

        if (fn.length < positions.length) {
            throw 'displaceArgs(:array, :fn) Given functions arity is to low';
        }
